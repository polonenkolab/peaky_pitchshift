...
Used to pick peaks of the modeled ABRs
"""
...
sv_root = f'{src}/peaks/{marker}/{marker}_{exp}_model' + \
    f'_{int(l_freq)}-{int(h_freq)}Hz'
overwrite_file = True
save_fig = True
save_hdf5 = True
save_txt = False
...
def onclose(event):
    ...
    print(amplitude)
    if save_fig:
        plt.savefig(sv_root + '.png')
    if save_hdf5:
        write_hdf5(sv_root + '.hdf5', dict(
    ...
    if save_txt:
        with open(sv_root + '.txt', 'w') as outfile:
...
ax.set_title('model (optimal params)')
counter = 0
...
for fi, f0 in enumerate(f0s):
    for pi, peak in enumerate(peaks):
        for si, [stim, sh] in enumerate(zip(stimuli, stim_shifts)):
            line, = ax.plot(
                t_ms[tmin:tmax] + sh,
                y_display * counter + w[si, fi, tmin:tmax],
                color=colors_stim[si][fi], lw=lw[0], alpha=1,
                picker=True, pickradius=picker_size[0],
                label=f'model_{stim}_{f0}_{peak}')
            text = ax.text(
                sh, y_display * counter + y_labelshiftprop * y_display,
                f'{f0}: {peak}', ha='left', va='bottom',
                color=colors_stim[si][fi])
