...
speech_root = 'stimuli/slices/speech_f0_10s_wavs.hdf5'
click_root = 'stimuli/slices/clicks_f0rates_10s.hdf5'
dpath = opath + 'data/'
...
#                 narr, f0, narr, ti, f0))
#             assert fs == fs_stim
wavs = read_hdf5(opath + speech_root)['wavs']

if do_click:
    clicks = read_hdf5(opath + click_root)['x_10s'][:, :n_trials_clicks]
...
with ExperimentController('peaky_pitch',
    ...
    ec.screen_prompt(instructions_speech.format(int(
            n_min_speech * n_f0s * n_narrs)), live_keys=['1'], wrap=True)
    start_times = [ec.current_time]
    tr_num = 0
    ...
    while not td_track.stopped:
        ...
        for ss, level in td_track:
            ...
            si = stim_index[narrators[ni]]
            trial_audio = np.tile(wavs[ss][ti], [2, 1])
            ec.identify_trial(
                ec_id='{}_{}f0_{:03}'.format(stimuli[si], f0s[fi], ti),
                ttl_id=decimals_to_binary(
                    [si, fi, ti], [n_bits_stim, n_bits_f0, n_bits_trial]))
            ec.load_buffer(trial_audio)
            # set display
            ttr = (stim_dur + pause_dur) * (n_trials_speech_total - tr_num)
            status_string = (
                '\nPlaying trial {} / {}\n'.format(tr_num + 1,
                                                   n_trials_speech_total) +
                '\nTotal time remaining: {} min\n'.format(int(ttr // 60)) +
                '\nFinish time: {}\n'.format(time.strftime(
                    '%I:%M', time.localtime(time.time() + ttr))))
            ec.screen_text(status_string + "\nPress '1' to pause.", wrap=True)
            pb_tot.update_bar((100. * (tr_num + 1)) / n_trials_speech_total)
            pb_tot.draw()
            ec.flip()
            # play the file
            start_times += [ec.current_time]
            ...
            tr_num += 1
            ec.check_force_quit()
    if tr_num == n_trials_speech_total:
        ec.wait_secs(5)
